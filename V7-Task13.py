...
class Stat:
    ...
    def report(self):
        print("\n===== ИТОГ =====")
        exp_tcp=BURST_PKTS   # ожидаем TCP-OUT на +1 с
        exp_udp=BURST_PKTS   #          UDP-OUT на +3 с
        got_tcp=self.tcp[1] if len(self.tcp)>1 else 0
        got_udp=self.udp[3] if len(self.udp)>3 else 0
        print(f"Ожидалось / поймано TCP @+1 с : {exp_tcp} / {got_tcp}")
        print(f"Ожидалось / поймано UDP @+3 с : {exp_udp} / {got_udp}\n")
